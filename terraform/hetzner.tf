...
resource "hcloud_server" "server_k8s_test" {
  ...
  ssh_keys     = [data.hcloud_ssh_key.macos.id]
  user_data = templatefile("${path.module}/modules/k8s-cluster/templates/cloud-init-k8s-manager.tftpl", {
    ...
    packages_setup : base64encode(file("${path.module}/modules/k8s-cluster/templates/install-packages.sh"))
